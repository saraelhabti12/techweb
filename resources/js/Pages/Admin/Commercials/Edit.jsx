...
import { ShieldCheckIcon, UserPlusIcon, EnvelopeIcon, PhoneIcon, BanknotesIcon, BriefcaseIcon } from '@heroicons/react/24/outline';

export default function Edit({ auth, commercial, projects = [] }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        commission_type: commercial.commission_type || 'percentage',
        commission_value: commercial.commission_value || 0,
        notes: commercial.notes || '',
        project_ids: commercial.projects ? commercial.projects.map(p => p.id) : [],
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('admin.commercials.update', commercial.id), {
            forceFormData: true,
        });
    };
    ...
    return (
        <AdminLayout auth={auth}>
            <Head title={`Edit Partner: ${commercial.name}`} />
            <DashboardPage 
                title="Edit Commercial Partner"
                description={`Updating information and commission logic for ${commercial.name}.`}
                actions={
            ...
            >
                <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white rounded-xl focus:ring-2 focus:ring-[#1F2BF3] px-4 py-3 shadow-sm transition-all"
                                    />
                                    <p className="mt-1 text-[9px] font-bold text-gray-400 uppercase">Auto-updates member link if email changes.</p>
                                ...
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1.5">Update Photo</label>
                                    <input
                                        type="file"
                    ...
                    <div className="space-y-6">
                        <DashboardCard title="Assigned Projects" description="Manage project links for this partner.">
